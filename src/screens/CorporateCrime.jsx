...
const CorporateCrime = () => {
  ...
  const handleNextQues = async () => {
    ...
    const response = await fetch(`${BACKEND}/api/questions/check-answer`, {
      ...
      body: JSON.stringify({
        userAnswer: val,
        questionId: currques.sequenceNumber,
      }),
    });
    const data = await response.json();
                ...
                <tbody>
                  {leaderboardData.length ? (
                    leaderboardData.map((player, index) => (
                      <tr
                        key={index}
                        className="border-b border-red-900/30 hover:bg-red-900/20"
                      >
                        ...
                        <td className="py-2 px-1 sm:px-4 text-right">
                          {formatTimeDifference(player.timestamp)}
                        </td>
                      </tr>
                    ))
                  ) : (
                ...
                {currques.sequenceNumber <= 6 ? (
                  <>
                    <p className=" max-md:text-base">
                      Location of Question : {currques.sequenceNumber}
                    </p>
                    <img
                      src={currques.destinationPic}
                      alt="abhi wali"
                      className="text-2xl sm:text-4xl font-bold mb-4 text-red-600 "
                    />
                    <div className="w-full flex justify-center items-center flex-col">
                      ...
                      <Button
                        variant="outlined"
                        color="error"
                        className="text-red-600 mt-5 px-5 py-2 font-semibold"
